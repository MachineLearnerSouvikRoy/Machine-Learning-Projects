...
movie <- read.csv("D:/Courses/ML in R/Files_svm_r/Files/Movie_classification.csv", header = TRUE)
...
df <- read.csv("D:/Courses/ML in R/Files_svm_r/Files/Movie_regression.csv", header = TRUE)
...
#radial
svmfitRad = svm(Collection~., data=train , kernel = "radial", cost =0.01, scale = TRUE )
summary (svmfit)

tune.outRad = tune(svm ,Collection~.,data=trainc ,kernel="radial", ranges = list(cost=c(0.001, 0.01,0.1 ,1 ,10 ,100 ,1000), gamma=c(0.01, 0.1,0.5,1,2,3,4,10, 50) ))
summary(tune.outRad)
bestmodR =tune.outR$best.model
summary (bestmodR)
ypredRad = predict (svmfit ,test)

mseRad <- mean((ypred-test$Collection)^2)
